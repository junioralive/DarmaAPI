...
import { Anime, Episode } from "../model/Anime";
...
export default class Anitaku {
  baseURL: string = "https://anitaku.to/";

  fetchInfo = async (id: string): Promise<Anime> => {
    var data: Anime = { id: id };
  ...
  search = async (q: string, page = "1"): Promise<Anime[]> => {
    var data: Anime[] = [];
    try {
  ...
  recent = async (page = "1"): Promise<Anime[]> => {
    var data: Anime[] = [];
    try {
      const res = await axios.get(
  ...
  fetchSources = async (
    id: string
  ): Promise<{ src: string; type?: string; referer?: string } | any> => {
    try {
